...
<div class="page-wrapper">
  ...
  <main>
    <nav class="nav">
      ...
      <?php  if(!empty($error_input)) {$classname ='form--invalid';} else {$classname ='';} ?>
    <form class="form form--add-lot container <?=$classname;?> " action="add.php" method="post"> <!--  $error_input['check'] === 'false' ? 'form--invalid' : ''  -->
      <h2>Добавление лота</h2>
      <div class="form__container-two">
          <?php  if(!empty($error_input['lot-name'])) {$classname ='form__item--invalid';} else {$classname ='';} ?>
      ...
      <div class="form__container-three">
          <?php  if(!empty($error_input['lot-rate'])) {$classname ='form__item--invalid';} else {$classname ='';} ?>
        <div class="form__item form__item--small <?=$classname?>">
          <label for="lot-rate">Начальная цена <sup>*</sup></label>
          <input id="lot-rate" type="text" name="lot-rate" placeholder="0" value="<?=getPostVal('lot-rate'); ?>">
          <span class="form__error"><?=$error_input['lot-rate']?></span>
        </div>
          <?php  if(!empty($error_input['lot-step'])) {$classname ='form__item--invalid';} else {$classname ='';} ?>
        <div class="form__item form__item--small <?=$classname?>">
          <label for="lot-step">Шаг ставки <sup>*</sup></label>
          <input id="lot-step" type="text" name="lot-step" placeholder="0" value="<?=getPostVal('lot-step'); ?>">
          <span class="form__error"><?=$error_input['lot-rate']?></span>
        </div>
          <?php  if(!empty($error_input['lot-date'])) {$classname ='form__item--invalid';} else {$classname ='';} ?>
        <div class="form__item <?=$classname?>">
          ...
          <span class="form__error"><?=$error_input['lot-date']?></span>
        </div>
      </div>
